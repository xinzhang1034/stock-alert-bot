...
def _normalize_pool_code(code) -> str:
    """修复：涨停池/龙虎榜的代码列有时会被 pandas 当成数字类型解析，导致前导 0 丢失
    （比如 '000001' 变成整数 1），用错误代码去拉行情必然全部失败。这里对纯num 且
    不足 6 位的代码补零到 6 位"""
    text = str(code).strip()
    if text.replace(".0", "", 1).isdigit():
        text = text.split(".")[0]  # pandas 有时会进一步解析成 float，去掉 .0 后缀
        if len(text) < 6:
            text = text.zfill(6)
    return text
...
def _build_stock_from_limit_up_row(row) -> dict:
    ...
    if not code:
        try:
            ...
            code = 'N/A'
    code = _normalize_pool_code(code)
