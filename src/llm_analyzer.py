...
def _parse_positive_number(value):
    """尽量宽松地把 take_profit_pct/stop_loss_pct 解析成正数：兼容数字类型、带 % 号、
    带多余空格的字符串；解析不出来返回 None（调用方只会把这一个字段置空，不会丢弃整条候选，
    因为 LLM 不一定严格按 prompt 里的示例格式返回，之前"格式不对就整条丢弃"太严格，
    容易把真实推荐误判成需要补充）"""
    if value is None:
        return None
...
_PRICE_RANGE_SEPARATORS = ("~", "-", "－", "—", "至", "to")


def _parse_price_range(value):
    """尽量宽松地把 buy_price_range 解析并规范化成 '低-高'；支持常见的分隔符变体
    （波浪号/全角横线/中文"至"等），解析不出来返回 None（同样只置空该字段，不丢弃整条候选）"""
    if not isinstance(value, str):
        return None
    text = value.strip()
    for sep in _PRICE_RANGE_SEPARATORS:
        if sep not in text:
            continue
...
def analyze_and_select_stocks(news_summary, limit_up_summary, dragon_tiger_summary):
    ...
    try:
        ...
        # 过滤掉不是 dict、缺少 stock_code 的畸形条目（这个必须整条丢弃，没有 stock_code
        # 下游没法用）；数值字段格式不对只置空该字段、不丢弃整条候选，避免把真实推荐误判成
        # 需要补充
        valid_stocks = []
        for item in stocks:
            ...
            take_profit = _parse_positive_number(item.get("take_profit_pct"))
            if take_profit is None and item.get("take_profit_pct") is not None:
                logger.warning(f"{item.get('stock_name')} 止盈百分比格式无法识别，展示为 N/A: {item.get('take_profit_pct')!r}")
            item["take_profit_pct"] = take_profit

            stop_loss = _parse_positive_number(item.get("stop_loss_pct"))
            if stop_loss is None and item.get("stop_loss_pct") is not None:
                logger.warning(f"{item.get('stock_name')} 止损百分比格式无法识别，展示为 N/A: {item.get('stop_loss_pct')!r}")
            item["stop_loss_pct"] = stop_loss

            price_range = _parse_price_range(item.get("buy_price_range"))
            if price_range is None and item.get("buy_price_range") is not None:
                logger.warning(f"{item.get('stock_name')} 买入价区间格式无法识别，展示为 N/A: {item.get('buy_price_range')!r}")
            item["buy_price_range"] = price_range
